...
def _article_topic(article) -> str:
    ...
    if any(k in base for k in ("tv", "stream", "ott", "vod")):
        return "TV/Streaming"
    if any(k in base for k in ("5g", "telco", "mobile", "network", "carrier", "operator")):
        return "Telco/5G"
    if any(k in base for k in ("media", "platform", "social", "adtech", "advertising")):
        return "Media/Platforms"
    if any(k in base for k in ("robot", "automation")):
        return "Robotics/Automation"
    if any(k in base for k in ("broadcast", "video", "linear")):
        return "Broadcast/Video"
    if any(k in base for k in ("satcom", "satellite")):
        return "Satellite/Satcom"
    if any(k in base for k in ("space", "orbital", "launch", "rocket")):
        return "Space/Infra"
